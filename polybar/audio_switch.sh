...
HEADSET_PATTERN='corsair|hs80|head(set|phone)|gaming'
SPEAKER_PATTERN='generic_usb_audio|usb audio|speaker|hdmi|pci-|analog-stereo'
...
get_headset_sink() {
    pick_sink_by_name "${PREFERRED_HEADSET_SINKS[@]}" || pick_sink_by_pattern "$HEADSET_PATTERN"
}
...
get_speaker_sink() {
    ...
    pick_sink_by_name "${PREFERRED_SPEAKER_SINKS[@]}" | grep -Fvx "$headset_sink" && return 0
    list_sinks | grep -Fvx "$headset_sink" | grep -Eim1 "$SPEAKER_PATTERN"
}
